...
%
% The time-dependent hamiltonian: H = -0.5*nabla^{2} + V(x) + F*x*cos(w*t). 
%
...
%
function [] = one_dim_soft_Coulomb_with_ac_field_harmonic_generation
clc; 
format short
%
...
%-0.6700   -0.2751   -0.1516   -0.0926   -0.0609   -0.0321    0.0006    0.0424    0.0896    0.1457
%           
...
%
    psi_old = psi_t0;
    H_ham = -0.5.*u_mat + diag(V_pot) + diag(H_pert).*cos(omega.*t(i));    
...
%    dipole_ave = sum(conj(psi_t0).*x(2:N).*psi_t0.*dx);    
    dipole_ave = sum(conj(psi_t0).*(x(2:N)./(1.+x(2:N).^2).^(3/2)).*psi_t0.*dx);        
    %
    output = [i*dt, dipole_ave];
    %
    fprintf(fileID_save_data_1, '%4.4f \t %8.16f\n', output); 
