...
%
% The occupation probability P_{1} = |<psi(x,t)|psi_{1}>|^{2} is obtained as function of time. 
%
...
%
function [] = one_dim_har_ocs_with_ac_field_tdse_n1
clc; 
format long 
%
...
%%%%%%%%%%%%%%%%%%%%%%%%
fileID_save_data_1 = fopen('one_dim_har_ocs_with_ac_fiel_dtdse_n1.txt','w');
%%% time loop starts ---
for i = 1:time_iter
    t(i) = i*dt;
%
    ...
    psi_t0 = psi_new;
    %
    N_popul_decay = abs(sum(conj(psi_t0).*V2).*dx).^2;    
    %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
read_popul_data = fopen('one_dim_har_ocs_with_ac_fiel_dtdse_n1.txt', 'r');               % 
read_popul_data = textscan(read_popul_data, '%f %f');
ho_time = read_popul_data{1};
ho_popul_n1 = read_popul_data{2};
%

figure(1)
plot(ho_time, ho_popul_n1, 'b', 'LineWidth', 1.5)
xlabel('\mbox{Time} (au)','Interpreter','latex') % ,'fontsize',16
ylabel('$|\langle \psi(x,t)|\psi_{1}(x)\rangle|^{2}$','Interpreter','latex') % , 'Rotation',0
%axis([0. 3. 0.000 2.00])
set(gca,'FontSize',16)
box on
